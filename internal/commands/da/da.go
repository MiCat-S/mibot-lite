...
func (r *daRun) retryAfter(err error, event, failure string) bool {
	if wait, ok := tgerr.AsFloodWait(err); ok {
		return kit.Sleep(r.ctx, wait) == nil
	}
...
func (r *daRun) sweepAll() bool {
	...
	for {
		...
		if kit.Sleep(r.ctx, 100*time.Millisecond) != nil {
			return false
		}
	}
	if len(batch) > 0 {
...
func (r *daRun) sweepOwn() bool {
	...
	for {
		...
		if kit.Sleep(r.ctx, 200*time.Millisecond) != nil {
			return false
		}
	}
	return true

...
func parseTranslation(raw []byte) (*translateResult, error) {
	...
	if err := json.Unmarshal(raw, &outer); err != nil || len(outer) == 0 {
		return nil, kit.Fail("翻译服务返回了无法解析的内容")
	}
	var plain string
	if err := json.Unmarshal(outer[0], &plain); err == nil {
		if strings.TrimSpace(plain) == "" {
			return nil, kit.Fail("翻译结果为空")
		}
...
func runTranslate(ctx context.Context, inv *command.Invocation, settings *store.Store[translateConfig]) error {
	...
	if first == "set" {
		...
		if !ok {
			return kit.Failf("不认识的语言代码：%s。可用代码见 %str help", inv.Arg(1), inv.Prefix)
		}
		if err := settings.Update(func(c *translateConfig) error { c.Target = code; return nil }); err != nil {
			return err
		}
		return inv.Edit(ctx, kit.Feedback("success", "默认目标语言已设置", languageName(code)+"（"+code+"）"))
	}
	...
	if kit.UTF16Len(text) > translateLimit {
		return kit.Failf("文本过长，请保持在 %d 字符以内", translateLimit)
	}
	if !named {
	...
	if err := inv.Edit(ctx, kit.Feedback("working", "翻译中", "")); err != nil {
		return err
	}
	...
	preview := kit.TruncateRunes(text, 60)
	suffix := ""
	if len([]rune(text)) > 60 {
		suffix = "…"
	}

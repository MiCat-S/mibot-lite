...
// Delegate 注册 .sudo 和 .sure，并在应用里挂上看别人消息的监听者。
// 先问 sure 再问 sudo：sure 的规则更具体，还可能带重定向。
func Delegate(a *app.App) {
	sudoList := newStore(a, "sudo.json", func() delegateDocument { return delegateDocument{} })
	sureList := newStore(a, "sure.json", func() delegateDocument { return delegateDocument{} })

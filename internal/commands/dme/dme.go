// Package dme 实现 .dme：删除自己的消息，可以先改成占位内容再删（防撤回）。
package dme
	...
	if !run.direct && isChannel {
		if result, err := run.api.ChannelsGetSendAs(ctx, &tg.ChannelsGetSendAsRequest{Peer: peer}); err == nil {
			for _, item := range result.Peers {
				if key := kit.PeerKey(item.Peer); key != "" {
...
func (r *dmeRun) retry(fn func() error) error { return kit.RetryFlood(r.ctx, r.cfg.RetryAttempts, fn) }
...
func (r *dmeRun) mine(m *tg.Message) bool {
	...
	if from, ok := m.GetFromID(); ok {
		...
		return r.identities[kit.PeerKey(from)]
	}
	return false
}
...
func (r *dmeRun) nextPage(limit int) ([]tg.MessageClass, pageOutcome, error) {
	...
	err := kit.RetryFlood(r.ctx, 2, func() error {
		var err error
	...
	for _, item := range page {
		if id := kit.MessageID(item); id > 0 && (next == 0 || id < next) {
			next = id
		}
	}
...
func dmeExecute(ctx context.Context, inv *command.Invocation, count int, anti bool, cfg dmeConfig) error {
	...
	for unlimited || run.matched < target {
		...
		run.forgetPassed()
		if err := kit.Sleep(ctx, 100*time.Millisecond); err != nil {
			return err
		}
	}
	run.report(count)
